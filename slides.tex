...
\begin{frame}
    ...
    \framesubtitle{Wong's equations of motion}
        \setbeamertemplate{itemize item}{\raisebox{0.2em}{\scalebox{0.7}{${\color{ming}\blacktriangleright}$}}} 
        \begin{itemize}
            \item \begin{center}{\Large{\color{ming}Approach}: {\color{ming}numerical trajectories} of classical particles in glasma fields} \end{center}
        \end{itemize} 
   \begin{center}
...
\begin{frame}[t]
    ...
    \begin{center}
        \begin{tikzpicture}[xscale=1]%[scale=0.9, every node/.style={scale=0.6}]
            ...
            \foreach \X [evaluate=\X as \Y using int(\X-2017),count=\Z] in {2018, 2019, 2021, 2022, 2023, 2024}
            {
    ...
    \begin{columns}[onlytextwidth,t]
       ...
       \begin{figure}
        ...
        \begin{center}
            \begin{itemize}
                \item<1> D-meson $R_{pA}$\only<1>{\footnotemark \\ {\color{lightgray}\footnotesize FONLL input + fragmentation\\
                SU(2) glasma, static box}}
                \item<2> Hybrid $R_{AA}$ and $v_2$\only<2>{\footnotemark \\ {\color{lightgray}\footnotesize Compared with Fokker-Planck diffusion\\
                SU(2) glasma, static box}}
                \item<3> Momentum variance $\sigma_p$\only<3>{\footnotemark \\ {\color{lightgray}\footnotesize Compared with Langevin diffusion\\
                SU(2) glasma, static box}}
                \item<4> Momentum variance $\sigma_p$\only<4>{\footnotemark \\ {\color{lightgray}\footnotesize SU(2) glasma, longitudinal expansion}}
            \end{itemize}
        \end{center}
        \column{.02\textwidth}
    \end{columns}
...
\setbeamertemplate{background}{
\tikz[overlay,remember picture] \node[opacity=0.1, at=(current page.center), align=center] {\\[10pt]
{\transparent{0.2}\includegraphics[height=0.7\paperheight]{images/heavy-ion3.pdf}}
   \\[10pt]  
